package viper.silver.inference.v1

import viper.silver.ast._


case class PermInf(program: Program) {
  ...
  case class Ident(name: String, typ: Type) extends Term {
    def instantiate(vars: VariableInstantiation): Term = {
  ...
  case class FieldAcc(base: Term, field: String, typ: Type) extends Term {
    def instantiate(vars: VariableInstantiation): FieldAcc = {
      FieldAcc(this.base.instantiate(vars), this.field, this.typ)
    }

    def pretty(): String = {
      s"${this.base.pretty()}.${this.field}: ${this.typ}"
    }
  }
  ...
  def locToTerm(loc: Exp): Term = {
    ...
    loc match {
      ...
      case localVar: AbstractLocalVar => localVar match {
        case p@LocalVar(name, _) => Ident(name, p.typ)
        case _ => {
          throw new IllegalArgumentException(s"Unknown expression type ${loc.getClass.getName}")
        }
      }
  ...
  def expToTerm(exp: Exp): Term = {
    exp match {
      ...
      case p@FalseLit() => BoolTerm(false, p.typ)
      case p@TrueLit() => BoolTerm(true, p.typ)
      case _ => {
  ...
  def collectAccessRequirements(exp: Exp): PermissionRequirements = {
    exp match {
      ...
      case p@PredicateAccessPredicate(pred, permExp) => {
      ...
      case access: LocationAccess => access match {
        case p@FieldAccess(l@LocalVar(name, _), field) => {
          PermissionRequirements(Set(FieldAcc(Ident(name, l.typ), field.name, p.typ)), Set())
        }
        case p@FieldAccess(rcv, field) => {
          val subs = collectAccessRequirements(rcv)
          if (subs.fields == 1) {
            val fa = subs.fields.toSeq.head
            PermissionRequirements(Set(FieldAcc(fa, field.name, p.typ), fa), Set())
          }
          else {
  ...
  def faToFA(fieldAccess: FieldAccess): FieldAcc = {
    FieldAcc(expToTerm(fieldAccess.rcv), fieldAccess.field.name, fieldAccess.typ)
  }

  def computeStartingTPT(exp: Exp): TransparentPredicateTree = {
    exp match {
      case predicate: AccessPredicate => predicate match {
        case p@FieldAccessPredicate(loc, permExp) => {
          TransparentPredicateTree(Set(FieldAcc(expToTerm(loc.rcv), loc.field.name, p.typ)), Set())
        }
  ...
  def synthesizeFoldingInstructions(strategy: FoldingStrategy) : Seq[Stmt] = {
    strategy.steps.map(s => {
      val acc = PredicateAccess(s.pred.values.map(t => termToExp(t)), s.pred.name)()
      val pred = PredicateAccessPredicate(acc, None)()
      if(s.unfolding){
        Unfold(pred)()
      }
      else{
  ...
  def inferMethod(defs: Map[String, PredDef], method: Method, depth: Int): Method = {
    println("===============================================================")

    defs.values.foreach(d => println(d.pretty()))
    ...
    val updatedBody = method.body match {
      case Some(value) => {
        val afterProcessing = value.ss.foldLeft((startingTPT, Seq[Seq[Stmt]]()))((acc, stmt) => {
          val tpt = acc._1
          val processed = acc._2
          // map each statement to the folding and unfolding operations
          println(s"${tpt.pretty()}")
          println(s">> ${stmt}")
          val processingResult: (TransparentPredicateTree, FoldingStrategy) = stmt match {
            case p@NewStmt(lhs, fields) => {

              val adjustedTPT = fields.foldLeft(tpt)((t, f) => {
                t.addFieldAccessPerm(FieldAcc(Ident(lhs.name, lhs.typ), f.name, f.typ))
              })
              (adjustedTPT, FoldingStrategy(Seq()))
            }
            case MethodCall(name, args, targets) => {
              ...
              strats match {
                case Some(value) => {
                  val unfolded = applyUnfoldingStrategies(defs, tpt, value)
                  val retInit = VariableInstantiation(method.formalReturns.map(f => f.name).zip(targets.map(l => Ident(l.name, l.typ))).toMap)
            ...
            case assign: AbstractAssign => assign match {
              case LocalVarAssign(lhs, rhs) => {
                ...
                reqs match {
                  case Some(value) => (applyUnfoldingStrategies(defs, tpt, value), value)
                  case None => {
                    sys.error("Unable to find unfolding strategy for all requirements!")
                  }
                }
          ...
          println("folding strategy: " + processingResult._2.pretty())
          val synthesizedInstructions = synthesizeFoldingInstructions(processingResult._2)

          println(s"SYNTH: ${synthesizedInstructions}")

          (processingResult._1, processed ++ Seq(synthesizedInstructions ++ Seq(stmt)))
        })

        println(s"${afterProcessing._1.pretty()}")
        afterProcessing._2.flatMap(i => i).foreach(v => println(">>" + v))


        val requirements = method.posts.map(collectAccessRequirements)
          .foldLeft(PermissionRequirements(Set(), Set()))((a, b) => a.merge(b))
        println(s"CURRENT TPT: ${afterProcessing._1.pretty()}")
        println(s"POST REQUIREMENTS: ${requirements}")

        val strats = getUnfoldingStrategiesForAllRequirements(depth, afterProcessing._1, defs, requirements)
        println(s"REFOLDING STRAT: ${strats}")

        strats match {
          case Some(postStrat) => {
            val synthesizedInstructions = synthesizeFoldingInstructions(postStrat)
            val adjustedStmts = afterProcessing._2 ++ Seq(synthesizedInstructions)

            Some(Seqn(adjustedStmts.flatten, value.scopedSeqnDeclarations)(pos=value.pos, info=value.info, errT=value.errT))
          }
          case None => None
        }
      }
    ...
    if(updatedBody.isDefined != method.body.isDefined) sys.error("Method inference failed!")
    Method(method.name, method.formalArgs, method.formalReturns, method.pres, method.posts, updatedBody)(pos=method.pos, info=method.info, errT=method.errT)
  }

  def termToExp(t: Term) : Exp = {
    t match {
      case BoolTerm(value, _) => {
        if(value){
          TrueLit()()
        }
        else{
      ...
      case FieldAcc(base, field, typ) => {
        FieldAccess(termToExp(base), Field(field, typ)())()
      }
      case FracPerm(left, right, _) => {
        FractionalPerm(termToExp(left), termToExp(right))()
      }
      case Ident(name, typ) => LocalVar(name, typ)()
      case IntTerm(value, _) => IntLit(value)()
      case _ => {
        throw new IllegalArgumentException(s"Unknown term to convert to exp ${t.getClass.getName}!")
      }
    }
  }
  ...
  def process(): Option[Program] = {
    try {
      ...
      /*
      TODO CFG: include type annotations in term graph
      */
      /*
      ...
      */
      ...
      val ddd = Seq(
        ("A", PredDef("A", Seq("this"), TransparentPredicateTree(
          Set(FieldAcc(Ident("this", Ref), "value", Ref)),
          Set()
        ))),
        ("B", PredDef("B", Seq("this"), TransparentPredicateTree(
          Set(FieldAcc(Ident("this", Ref), "value", Ref)),
          Set(PredInstance("C", Seq(Ident("this", Ref))))
        ))),
        ("C", PredDef("C", Seq("this"), TransparentPredicateTree(
          Set(FieldAcc(Ident("this", Ref), "additional", Ref)),
          Set()
        )))
      ).toMap
      println("PREDICATE DEFINITIONS:")
      println(ddd.map(d => d._2.pretty()).mkString("\n"))
      val ttt = TransparentPredicateTree(Set(FieldAcc(Ident("x", Ref), "additional", Ref)), Set(PredInstance("A", Seq(Ident("x", Ref)))))
      val result = ttt.findRefoldingStrategy(ddd, PredInstance("B", Seq(Ident("x", Ref))), 10)
      ...
      val tpt = TransparentPredicateTree(Set(), Set(PredInstance("Cont$$_$$$$_T$$$$_", Seq(Ident("y", Ref)))))
      println("--------------------")
      println(s"PRED DEF RESULT: ${defs}")
      println("--------------------")
      println(s"STARTING WITH: ${tpt}")
      val res = tpt.findUnfoldingStrategy(defs, FieldAcc(Ident("y", Ref), "Cont$$_$$$$_T$$$$_$$$value", Ref), 10)
      println(s"UNFOlDING STRATEGY RESULT: ${res}")
      println("--------------------")
      val updatedMethods = program.methods.map(m => {
        val updated = inferMethod(defs, m, 10)
        println("synthesized folding instructions")
        println(updated)
        updated
      })
      println("--------------------")
